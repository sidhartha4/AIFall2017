...
def getMLPOutput(x, W, b):
    ...
    layer1_sig = 1./(1.+np.exp(-layer1_op))

    return layer1_op, layer1_sig
...
def getGradient(x, y, l1s):
    ...
    output = getOutputVector(y)

    grad = l1s-output
    sigmoid_grad = l1s*(1-l1s)*grad
    del_b = sigmoid_grad
    ...
    for epoch in range(no_epochs):
        ...
        for batch in range(batches):
            ...
            for sample in range(mini_batch_size):
                l1, l1s = getMLPOutput(training_samplesX[sample], W, b)
                del_W, del_b = getGradient(training_samplesX[sample], training_samplesY[sample], l1s)                
                delta_W += del_W
                delta_b += del_b
            
            delta_W = delta_W/mini_batch_size
            delta_b = delta_b/mini_batch_size
...
def test():
    ...
    for x,y in zip(testX,testY):
        total += 1
        l1, l1s = getMLPOutput(x, W, b)
        classified = getLabelFromOutput(l1s)
